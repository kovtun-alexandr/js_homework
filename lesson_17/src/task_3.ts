...
if (confirm('Почати тестування?')) {
    ...
    class Storage {
        ...
        constructor(initProductsList: Product[] = []) {
         ...
         */
        public getSorting(): Product[] {
            return this.productList.reduce((acc: Product[], curr) => {
                const found = acc.find(
                    p => p.name === curr.name && p.company.name === curr.company.name
                )
                if (found) {
         ...
         */
        public shipProduct(productName: string, companyName: string, weight: number) {
