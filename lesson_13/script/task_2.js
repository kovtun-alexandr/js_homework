...
if (confirm('Почати тестування?')) {
    const words = ['Dog', 'Sun', 'Book', 'Tree', 'Milk'];
    const answers = ['Собака', 'Сонце', 'Книга', 'Дерево', 'Молоко'];
    const descriptions = [
     ...
     */
    function getRandomСollection(words, answers, descriptions) {
    ...
    function game(arr) {
         ...
         */
        function makeHintIterator(word) {
            let index = 0;
            return function () {
                if (index <= word.length) {
                    const part = word.slice(0, index);
        ...
        let letterFromIterator = nextLetter();
        while (letterFromIterator !== 'Слово закінчилось!') {
            const hint = nextHint();
            const userInput = prompt('Введіть букву:') || '';
